...
<!-- Reihenfolge und Vollständigkeit sind hier nicht Geschmack: Firefox fordert
     von diesen Zeilen NUR das SVG an und fällt von sich aus auf nichts zurück.
     Scheitert das SVG, bleibt der Tab leer — genau das war der Fall, solange
     icon.svg einen doppelten Bindestrich im Kommentar trug und damit als XML
     unlesbar war. Die .ico steht deshalb ausdrücklich dabei: sie ist das
     Format, das jeder Browser lesen kann, und /dashboard/favicon.ico gibt es
     nicht — der automatische Griff ans Wurzelverzeichnis läuft von einer Seite
     unter /dashboard/ ins Leere. -->
...
{body}
  <footer>
    ...
    <div class="fbar">
      ...
      <div class="fright fright-wrap">
        <span><span id="foot-ver-label">code</span> v.<code id="code-ver">–</code> (BETA)</span>
        <button class="fwake" id="wake-btn" type="button" aria-pressed="false" hidden><span class="wdot" aria-hidden="true"></span><span class="wtext">Screen stays on</span></button>
      </div>
    </div>
  </footer>
</div>
...
STYLE = """
  *{box-sizing:border-box}
  ...
  /* Wachhalten-Schalter. Diese Seite laedt theme.css nicht (eigene Palette),
     also stehen die Regeln hier noch einmal -- gleiche Optik, eigene Variablen. */
  .fright-wrap{display:flex;align-items:center;justify-content:flex-end;gap:10px}
  .fwake{display:inline-flex;align-items:center;gap:7px;border:1px solid var(--border);
    border-radius:999px;background:var(--surface);color:var(--muted);font:inherit;
    font-size:11.5px;line-height:1;padding:5px 10px;cursor:pointer;white-space:nowrap}
  .fwake[hidden]{display:none}
  .fwake:hover{color:var(--text);border-color:var(--accent)}
  .fwake .wdot{width:7px;height:7px;border-radius:50%;background:var(--faint);flex:none}
  .fwake[aria-pressed="true"]{color:var(--accent);border-color:var(--accent)}
  .fwake[aria-pressed="true"] .wdot{background:var(--accent)}
